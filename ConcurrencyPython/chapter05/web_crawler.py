...
class Crawler(threading.Thread):
    ...
    def crawl(thread_name, url, linksToCrawl):
        try:
            ...
            if (urlparse(link).netloc == 'tutorialedge.net') and (link not in Crawler.craledLinks):
                ...
                print("Url {} Crawled with Status: {} : {} Crawled In Total".format(
                    response.geturl(),response.getcode(), len(Crawler.crwaledLinks)))

                soup = BeautifulSoup(response.read(), "html.parser")
                Crawler.enqueueLinks(soup.find_all('a'), linksToCrawl)
        except URLError as e:
            print("URL {} threw this error when trying to parse: {}".format(
                link, e.reason
            ))
